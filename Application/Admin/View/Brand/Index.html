				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<h3 class="page-title">
							...
							品牌管理 <small>今日任务：统计在售商户商品的库存</small>
						...
						<ul class="breadcrumb">
							...
							<li style="float:right;padding-right:20px;"><a href="{:U('Admin/Brand/addbrand')}">添加品牌</a></li>
				...
				<tr>
					...
					<foreach name='brand' item='v'>
						...
						<tr onmouseover="this.style.backgroundColor='#ffff66';" onmouseout="this.style.backgroundColor='#FFF5EE';">
							...
							<td>{$v.dateline|date='y-m-d H:i',###}</td>

							<td>{$v.description}</td>

							<td><if condition='$v["status"]'>开启<else/>关闭</if></td>

							<td><a href="{:U('Admin/Brand/editbrand',array('id' => $v['id']))}">修改</a> <a onclick="if(confirm('是否要删除？'))window.location.href='{:U('Admin/Brand/delbrand',array('id' => $v['id']))}'">删除</a></td>
